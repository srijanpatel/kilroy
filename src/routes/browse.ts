...
browseRouter.get("/", async (c) => {
  ...
  const conditions: any[] = [eq(posts.projectId, projectId)];
  ...
  if (!recursive) {
    response.subtopics = await getSubtopics(projectId, topic, status);
  }
...
async function getSubtopics(
  ...
  const params: any[] = [prefix + "%", projectId, prefixLen];
  let statusCondition = "";
  if (status !== "all") {
    params.push(status);
